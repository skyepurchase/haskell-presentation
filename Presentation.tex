...
\begin{frame}{Introduction}
    Haskell is a purely functional programming language. \textbf{No side effects, not even printing.}
...
\begin{frame}[fragile]{Syntax \& Types: The Basics}
    Everything in Haskell is a function. And all functions in Haskell have a type.
    Modules are in PascalCase and functions are in camelCase.
...
\begin{frame}[fragile]{Syntax \& Types: Types}
    Types determine what the expected input and output will be. These can be strict and static (Java, C++) or loose and dynamic (Python, Typescript).

    The benefits of Haskell are
    \begin{itemize}
        \item \textbf{Type inference} The type checker can figure out exactly what type some data should.
        \item \textbf{Type classes} Similar to a \texttt{enum} except for types and can be used in \texttt{match} statements.
        \item \textbf{Higher-order types} A ``functions" that take in a type and produce a brand new type.
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Syntax \& Types: Monads}
    Haskell has a lot of jokes about monads ``monads are monoids in the category of endofunctors" or ``monads are just borritos". \\
...
\begin{frame}{Conclusion: Why use Haskell?}
    \begin{itemize}
        \item Haskell an incredibly powerful language with a very steep learning curve.
        \item The strong typing and functional nature make it perfect for safe multi-threading.
    \end{itemize}
\end{frame}
